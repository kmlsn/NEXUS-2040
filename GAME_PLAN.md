...
| Sonraki görev | `P3.6` En az yedi günlük sözleşme arşivini ve kaçırmama davranışını ekle |
| Kodlama durumu | P3.5 sunucu-otoriteli snapshot, deterministik teklif ve atomik teminat uzlaşmasını tamamladı; P3.6 süre/expiry/arşiv yaşam döngüsünü ekleyebilir |
...
- [x] **P3.4** Hikâye sözleşmesi ile teklif yarışlı pazar sözleşmesini ayır.
- [x] **P3.5** Sözleşme teklif puanı ile sınırlı teminat uzlaşmasını uygula; kaynak üreten başarısızlık analiz ödülünü P4.7'ye bırak.
...
| D-030 | P3.4 yalnız sürümlü `story` ve `market` sözleşme türlerini ve saf görünürlük ayrımını kurar; sözleşme örneği, A01-A06 kampanya içeriği, teklif puanı, fair value, teminat, süre/expiry, arşiv, olay günlüğü ve ödül eklemez. Hikâye sözleşmesinin görünürlüğü yalnız sunucu-türetilmiş kampanya uygunluğuna bağlıdır; pazar/teklif/RNG sonucu onu gizleyemez. Pazar sözleşmesinin görünürlüğü yalnız sunucu-türetilmiş pazar-listesi uygunluğuna bağlıdır. | Kabul | P3.5 kalıcı teklifli sözleşme panosu veya P6.1 kampanya içeriği eklenirken sürümlü örnek/snapshot, görünürlük kaynağı, zaman penceresi ve ilgili migration/ledger kanıtı birlikte tasarlanır. |
| D-031 | P3.5, oyuncuya açık bir pano kurmadan yalnız sunucu-içi `market` sözleşme snapshot'ı ve tek teklif uzlaşmasını kurar; listeleme/expiry/arşiv P3.6, hedef/operasyon/başarısızlık analiz ödülü P4.1/P4.7, kampanya metni P6.1'dir. Snapshot tekil `market` kimliği, `counterparty_organization_id='nexilune_industrial'`, dünya içerik/formül/revizyonu, pazar endeksi, `tier=1`, `BASE_REWARD_MICRO=1_000_000_000` (1000 Sermaye) ve `fair_value_micro=roundHalfAway(BASE_REWARD_MICRO*market_index_bp/10_000)` taşır; bid, fair value, hold ve refund yalnız mikro-Sermaye birimindedir. `price_score` 28.4 formülünün `[0,100]`e sıkıştırılmış, 9 ondalık half-away sabit-nokta sonucudur; teklif olasılığı bundan hesaplanır ve eşik 28.0'daki 9-onluk kural ile üretilir. `preparedness=round(clamp(0,100,100*sum(facility_levels)/60))`, `reputation=round(clamp(0,100,(nexilune_relation_tenths+1000)/20))`, `urgency_fit=round(clamp(0,100,(market_index_bp-8500)/30))`; `best_npc_score=round((Nexilune.bid_capacity+readiness)/2)` ve 28.4 teklif formülü yalnız sunucuda snapshotlanır. İstemci gelecekte yalnız kanonik pozitif `bid_micro` ve idempotency anahtarı gönderebilir; profil, skor, zaman, seed, NPC değeri veya sonuç gönderemez. Teklifte `held_micro=min(roundHalfAway(BASE_REWARD_MICRO*(5+3*tier)/100),roundHalfAway(liquid_capital_micro*20/100))`; `held_micro<4` ise teklif reddedilir. Hold, sonuç ve iade aynı PostgreSQL transaction'ında D-017 mikro-Sermaye ledger hareketiyle uygulanır; aynı teklif/event ikinci hareket üretemez. `contract:<contract_id>:offer:<profile_id>:award` PCG stream'i, çekiliş eşiği ve girdi snapshot'ı saklanır. P3.5'te award `%100`, kaybedilen teklif `%75` iade alır; sistem iptali/terk geçişleri P3.6/P4'e bırakılır. Kayıp teklif yeni kaynak/Uzmanlık üretmez: operasyon başarısızlığının `%25` analiz/değeri ve tespit çarpanı yalnız P4.7'de uygulanır. Hikâye görünürlüğü teklif/RNG ile değişmez. | Kabul | P3.6 gerçek liste/sona erme/arşiv veya P4 operasyon durumu, yeni bir terminal geçiş ya da kaynak ödülü gerektirirse ilgili sahibinde snapshot, ledger reason, event kimliği, seedli fixture ve gerçek-ledger denge kanıtı birlikte tasarlanır. |
...
| 2026-08-17 | P3.4 | D-030 `story`/`market` türleri ve saf, sunucu-türetilmiş görünürlük ayrımı; iki tür için tam `storyEligible × marketListed` matrisi ve geçersiz tür reddi; `pnpm verify`, güvenlik/kalite incelemeleri ve `docs/test-reports/P3.4-lifecycle.md` PASS | Geçti |
| 2026-08-17 | P3.5 | D-031 server-içi Nexilune market snapshot'ı, dokuz ondalık half-away teklif puanı, sürümlü PCG award stream'i ve atomik mikro-Sermaye hold/refund ledger uzlaşması; 015 migration ileri/geri/race/replay korumaları, TypeScript/Python sınır fixture'ı (`857600000/603 → 70313`), 500k sabit-seed + 30-seed award dağılım kanıtı; `pnpm verify`, CI `lint/typecheck/unit/integration`, mimari/denge/güvenlik/kalite incelemeleri ve `docs/test-reports/P3.5-lifecycle.md` PASS. P4.7 öncesi analiz/Uzmanlık ödülü eklenmedi. | Geçti |
