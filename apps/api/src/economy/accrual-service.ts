...
/** Server-only, transactionally idempotent lazy-accrual settlement. */
export class LazyAccrualService {
  ...
  async settle(profileId: string, asOfMs: bigint): Promise<AccrualOutcome> {
    ...
    try {
      ...
      if (rows.rowCount === 0) { await client.query("COMMIT"); return { settled: false, elapsedMs: 0n, deltas: {} }; }
      ...
      if (asOfMs <= startMs) { await client.query("COMMIT"); return { settled: false, elapsedMs: 0n, deltas: {} }; }
      ...
      for (let offset = 0n; offset < elapsedMax;) {
        ...
        const nextStart = rows.rows
          .map((row) => elapsedMax - (windows.get(row.id) ?? 0n))
        ...
        const bankRate = availableEnergy >= energyNeeded
          ? totalDemand
          : (availableEnergy / duration) * 3_600_000n + ((availableEnergy % duration) * 3_600_000n) / duration;
        ...
        for (const allocation of resolution.allocations) {
          ...
          const outputRate = rateFrom(allocation);
          const local = state.get(row.id);
          if (!local) throw new Error("Missing accrual state.");
          const output = accrueWithCanonicalCarry(outputRate, duration, local.outputCarry);
          ...
          if (row.facility_kind === "microgrid") availableEnergy += output.producedMicro;
          else availableEnergy -= consumption.producedMicro;
      ...
      if (Object.keys(serialized).length === 0) { await client.query("COMMIT"); return { settled: false, elapsedMs: elapsedMax, deltas: {} }; }
